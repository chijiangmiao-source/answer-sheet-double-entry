...
- 单元/组件测试：Vitest（34 个用例）
- 端到端测试：Playwright（5 个用例，覆盖通过、不通过、重新开始、批量填入）
...
3. 批量填入（扫描枪 / OCR）：
   ...
   - 关闭批量填入区不改变任何已录答案。
4. 任一轮存在漏题时**不能提交**：顶部汇总漏答题号，并在每一道漏答题的原位置标注“此题漏答”。
5. 首录完整提交后：
...
7. 结果页点击“重新开始”：清除两轮状态（含首录快照与裁决结果），返回空白的第一轮。
...
```bash
npm run test:unit  # Vitest：轮次隔离、完整性校验、裁决规则、键盘与原位提示、批量解析
npm run test:e2e   # Playwright：通过 / 不通过 / 重新开始 / 批量填入（自动起 preview 服务）
```
...
src/
  ...
  components/RoundView.vue # 单轮录入（键盘/鼠标/批量填入、漏题原位提示）
